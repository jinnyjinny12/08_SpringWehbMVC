...
// 재유닛이 붙이라고 해서 붙인 것: Tests
// 재유닛? - build.gradle 확인
class ProblemOfUsingDirectSqlTests {

    private Connection con;

    // testcode가 동작되기 전 항상 실행된다
    @BeforeEach
    ...
    // 1. 데이터 변환, SQL 작성, jdbc api 코드 등의 중복작성(개발 시간 증가, 유지보수성 저하)
    // displayname : 테스트 코드의 이름이다. 작성하지 않으면 클래스의 이름이 테스트코드의 이름이 된다
    ...
    // 등록은 되지만 데이터 베이스에 삽입은되지 않는다 ! ROLLBACK 처리 되어 있음
    ...
    void testChangeSelectColumns() throws SQLException {
        ...
        //when
        // List<Menu> 로 만들어져 있어서 SQL 값으로 초기화되지 않은 것들은 기본값으로 나옴(0, null)
    ...
    void testAssociatedObject() throws SQLException {
        ...
        while(rset.next()) {
            ...
            menuAndCategory.setOrderableStauts(rset.getString("ORDERABLE_STATUS"));
